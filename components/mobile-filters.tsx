...
export function MobileFilters({ open, onClose }: MobileFiltersProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent
        side="bottom"
        className="h-[90vh] w-full sm:w-96 sm:ml-auto p-0 rounded-t-xl overflow-hidden flex flex-col"
      >
            ...
            {categories.map((category) => (
              <div key={category} className="flex items-center justify-between py-2">
                ...
                <Checkbox
                  ...
                  onCheckedChange={() =>
                    dispatch(setCategory(filters.category === category ? "" : category))
                  }
                />
              </div>
            ))}
            ...
            <div className="grid grid-cols-5 gap-3">
              {colors.map((color) => (
                <button
                  ...
                  className={`w-10 h-10 rounded-full border-2 ${filters.colors.includes(color.name) ? "border-black" : "border-gray-300"
                    }`}
